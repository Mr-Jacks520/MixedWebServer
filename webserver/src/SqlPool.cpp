...
void SqlPool::Init(const char* host, unsigned int port, const char* user, const char* pwd, const char* dbName, const int maxconn) {
    ...
    errif(mysql_library_init(0, nullptr, nullptr) < 0, "[MYSQL]: could not initialize MySQL client library\n");
    // create source for pool
    for (int i = 0; i < maxconn; ++i) {
        MYSQL* sql = nullptr;
        // initializes a MYSQL object suitable for mysql_real_connect()
        sql = mysql_init(sql);
        errif(sql == NULL, "[MYSQL]: no sufficient memory for MYSQL object\n");
        // establish a connection to a MySQL server running on host
        sql = mysql_real_connect(sql, host, user, pwd, dbName, port, nullptr, 0);
        errif(sql == NULL, "[MYSQL]: connection was unsuccessful\n");
        _conns.push(sql);
    }
...
MYSQL* SqlPool::GetConn() {
    MYSQL* sql = nullptr;
    if (_conns.empty()) {
        LOG_ERROR("[MYSQL]: connn queue is busy.");
        return nullptr;
    }
    sem_wait(&_sems);
    {
