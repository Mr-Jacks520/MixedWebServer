...
sds HttpResponse::MakeResponse(sds buf)
{
    ...
    _ErrorPage();
    buf = _AddStatusLine(buf);
    buf = _AddHeader(buf);
    return _AddContent(buf);
}
...
sds HttpResponse::_AddStatusLine(sds buf)
{
    std::string status;
    ...
    return sdscatprintf(buf, "HTTP/1.1 %d %s\r\n", _code, status.c_str());
}

sds HttpResponse::_AddHeader(sds buf)
{
    ...
    return sdscatprintf(buf, "Content-Type: %s\r\n", _GetFileType().c_str());
}

sds HttpResponse::_AddContent(sds buf)
{
